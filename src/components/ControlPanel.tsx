...
export function ControlPanel() {
  ...
  // 新手指引：首次打开自动播放，走完或跳过标记已读；设置里可重看。
  const [guideOpen, setGuideOpen] = useState(!settings.onboardingCompleted);
  const handleGuideFinish = useCallback(() => {
    void saveSettings({ ...settings, onboardingCompleted: true });
    setGuideOpen(false);
  ...
  return (
    ...
    <section className="overview" aria-label="研究所概况">
      ...
      <button className="collection" data-guide="collection" aria-label={`打开发明档案馆，共 ${generation.cards.length} 张`}
    ...
    <button className={`invention-button ${generation.state}`} data-guide="invention-machine" onClick={handleGenerateCard}
    ...
    <OnboardingGuide open={guideOpen} onFinish={handleGuideFinish} />
      </main>
    </>
  );
}
