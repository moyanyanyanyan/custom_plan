...
const tasks: Task[] = [
  { id: '1', name: '整理桌面', icon: 'grid', minutes: 10, completed: true,
    group: 'A', createdAt: '2026-09-11T01:00:00Z', completedAt: '2026-09-11T02:00:00Z' },
  { id: '2', name: '提交周报', icon: 'book', minutes: 10, completed: false,
    group: 'A', createdAt: '2026-09-11T01:00:00Z', completedAt: null },
];

const setup = (pendingSlimeCount = 0) => {
  const handlers = { onAdd: vi.fn(), onToggle: vi.fn(), onRemove: vi.fn(),
    onOpenSlimes: vi.fn() };
  render(<TodayTaskBoard tasks={tasks} date={new Date(2026, 8, 11)}
...
describe('TodayTaskBoard', () => {
  ...
  it('只有复选按钮切换完成，点击正文只聚焦', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '提交周报待完成' }));
    expect(handlers.onToggle).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole('button', { name: '完成 提交周报' }));
    expect(handlers.onToggle).toHaveBeenCalledWith('2');
  });
  ...
  it('内联添加任务并保留删除确认', () => {
    ...
    expect(handlers.onAdd).toHaveBeenCalledWith('喝水');
    vi.spyOn(window, 'confirm').mockReturnValueOnce(true);
    fireEvent.click(screen.getByRole('button', { name: '删除 提交周报' }));
