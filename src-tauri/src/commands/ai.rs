use crate::{ai::prompts, data::AppStore};
use tauri::State;
...
pub async fn generate_card_copy(
    ...
    crate::ai::stepfun::generate_copy_with_store(system, user, &store)
...
pub async fn generate_card_art(
    ...
    let bytes = crate::ai::stepfun::generate_image_with_store(prompts::card_art(&name, &description), &store)
        .await.map_err(|e| e.to_string())?;
